...
> 對應右上角**帳號選單**（認證帳號下拉）與 professional 後台側欄的進入點。每個功能在真實平台的入口、對應檔案／路由，以及這份交接的 Demo。`(待補)` ＝ 真實平台尚無、需新增的入口。

| 功能 | 使用者入口（帳號選單 / 後台）| 真實平台路由 / 檔案 | 對應 Demo |
|---|---|---|---|
| 檢視創作者主頁 | 帳號選單 →「檢視我的創作者主頁」| `Profile.tsx`　`/[lang]/user/[uid]` | [`/creator`](https://internx-activity-handoff.zeabur.app/creator) |
| 主辦單位入口 | 帳號選單 →「主辦單位入口」| 取得主辦身份引導（**非必需**，可整併進設定）| [`/host`](https://internx-activity-handoff.zeabur.app/host) |
| 建立活動 | 帳號選單／後台 →「建立活動」| `professional/new-activity`、`AddActivity.tsx` | [`/editor`](https://internx-activity-handoff.zeabur.app/editor) |
| 撰寫新文章 | 帳號選單 →「撰寫新文章」| `BlockEditor`、`data/blog.ts` | [`/blog-editor`](https://internx-activity-handoff.zeabur.app/blog-editor) |
| 我的主辦活動 | 帳號選單 →「我的主辦活動」| professional 後台 / Profile「主辦活動」分頁（`createdBy`）| [`/backstage`](https://internx-activity-handoff.zeabur.app/backstage) 活動管理 |
| 報名名單 / 審核 | 後台側欄 →「報名名單」| `professional/registrations` `(待補)`；`registration.status` | [`/backstage`](https://internx-activity-handoff.zeabur.app/backstage) 報名名單 |
| 申請認證 | 帳號選單 →「申請認證」| `/dashboard/verified-role-apply`（入口 A，§10.1）| [`/register`](https://internx-activity-handoff.zeabur.app/register) |
| 探索創作者 | 主選單 →「探索創作者」| 創作者目錄 / directory | [`/directory`](https://internx-activity-handoff.zeabur.app/directory) |
| 工程交接文件 | 帳號選單 →「工程交接文件」| 本交接站（與 internx.me 分離）| [`/handoff`](https://internx-activity-handoff.zeabur.app/handoff)（本頁）|

> 說明：「主辦單位 ＝ 創作者」是**同一個認證帳號**（§10）。後台側欄的 **成果展示 / 回應評價** 為官方帳號**既有功能、沿用不需新做**（§11）；報名名單與審核狀態機為這次新增（§12）。
