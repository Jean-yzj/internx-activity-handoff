...
async function init() {
  ...
  try {
    ...
    // 負責人是後來加的；既有部署要就地補欄位，不然 assignee 的讀寫會炸
    await pool.query(`ALTER TABLE handoff_item_status ADD COLUMN IF NOT EXISTS assignee text`);
    await pool.query(`
  ...
  if (mode === 'pg') {
    try {
      const [s, n] = await Promise.all([
        pool.query('SELECT item_id, status, updated_by, updated_at, assignee FROM handoff_item_status'),
        pool.query('SELECT id, item_id, body, author, kind, resolved, created_at FROM handoff_notes ORDER BY id ASC'),
      ]);
      const items = {};
      s.rows.forEach((r) => {
        items[r.item_id] = { status: r.status, by: r.updated_by, at: r.updated_at, assignee: r.assignee || '' };
      });
...
// 狀態與負責人共用同一列。兩個 upsert 都只碰自己那幾欄，
// 所以改狀態不會清掉負責人，指派也不會把狀態打回未開始。
async function setStatus(itemId, status, by) {
  ...
  const d = fileRead();
  const prev = d.items[itemId] || {};
  d.items[itemId] = { status, by, at, assignee: prev.assignee || '' };
  fileWrite(d);
  return { itemId, status, by, at, assignee: prev.assignee || '' };
}

async function setAssignee(itemId, assignee, by) {
  const at = new Date().toISOString();
  if (mode === 'pg') {
    await pool.query(
      `INSERT INTO handoff_item_status (item_id, status, updated_by, updated_at, assignee)
       VALUES ($1,'todo',$2, now(), $3)
       ON CONFLICT (item_id) DO UPDATE SET assignee = EXCLUDED.assignee,
                                           updated_by = EXCLUDED.updated_by,
                                           updated_at = now()`,
      [itemId, by, assignee]
    );
    return { itemId, assignee, by, at };
  }
  const d = fileRead();
  const prev = d.items[itemId] || {};
  d.items[itemId] = { status: prev.status || 'todo', by, at, assignee };
  fileWrite(d);
  return { itemId, assignee, by, at };
}
...
module.exports = { init, getState, setStatus, setAssignee, addNote, resolveNote };
